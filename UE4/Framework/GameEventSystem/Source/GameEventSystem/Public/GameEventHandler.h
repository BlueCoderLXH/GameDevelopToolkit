...
private:
	...
	{
#ifdef UE_EDITOR
		UWorld* World = WorldContext ? WorldContext->GetWorld() : GEngine->GetCurrentPlayWorld();
		if (!World)
		{
			World = GWorld;
		}
		...
		return (World && (World->GetNetMode() != NM_Client)) ? ListenersOnDS : Listeners;
#else
		return Listeners;
#endif
	}
...
#ifdef UE_EDITOR
	FGameEventListener ListenersOnDS;
#endif
