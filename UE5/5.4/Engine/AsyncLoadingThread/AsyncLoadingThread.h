...
struct FAsyncLoadEvent
{
	...
	{
		NormalIsolation = 20,
		PriorityIsolation = NormalIsolation / 2,
		RequestPriority_MAX = MAX_int32
	};
...
struct FAsyncLoadEventQueue
{
	...
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	/** 
	 * [ASYNC THREAD] Elevate the priority of all events belonging to a package.
	 *
	 * @param InPackageSerialNumber The SerialNumber of the package whose events to elevate
	 * @param NewUserPriority       The new UserPriority to assign (typically PACKAGE_PRIORITY_HIGHEST)
	 */
	void RequeueEventsForPackage(int32 InPackageSerialNumber, int32 NewUserPriority)
	{
		int32 ElevatedCount = 0;
		for (FAsyncLoadEvent& Event : EventQueue)
		{
			if (Event.PackageSerialNumber == InPackageSerialNumber)
			{
				Event.UserPriority = NewUserPriority;
				++ElevatedCount;
			}
		}
...
//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
class FPriorityPackageLoadState
{
public:
	bool InitializeFromCommandLine();
	
	FORCEINLINE bool IsEnabled() const { return bEnabled && GEventDrivenLoaderEnabled; }
	
	FORCEINLINE void OnDescQueued() { PendingDescCount.Increment(); }
	
	FORCEINLINE void OnDescConsumed()
	{
		if (PendingDescCount.Decrement() < 0) { PendingDescCount.Increment(); }
	}	
	
	FORCEINLINE bool HasPendingDescs() const { return PendingDescCount.GetValue() > 0; }
	
	FORCEINLINE void ResetDescCount() { PendingDescCount.Reset(); }
	
	FORCEINLINE bool HasReadyPackages() const { return ReadyForTick.Num() > 0; }
	
	FORCEINLINE bool HasPendingPackages() const { return PendingPackages.Num() > 0; }

	FORCEINLINE void SignalPackageLoaded() { if (IsEnabled()) bPriorityPackageLoaded = true; }

	FORCEINLINE void ClearPackageLoadedSignal() { bPriorityPackageLoaded = false; }

	FORCEINLINE bool HasLoadedPackage() const { return bPriorityPackageLoaded; }

	void OnPackageAdded(const FAsyncPackage* Package);

	void OnPackageReleased(const FAsyncPackage* Package);

	static bool IsPriorityPackage(const FAsyncPackage* Package);
	
	static bool IsPriorityPackage(const FAsyncPackageDesc& Package);

	int32 AdjustEventPriority(const FAsyncPackage* Package, int32 Original) const;

	bool ShouldYield(FAsyncPackage* Package, EAsyncPackageYieldPoint::Type YieldPoint) const;

	void AddReadyPackage(FAsyncPackage* Package);

	void RemoveReadyPackage(FAsyncPackage* Package);

	FAsyncPackage* PopNextReadyPackage(FAsyncPackage* NormalQueueHead);

	void SortLoadedPackages(TArray<FAsyncPackage*>& InProcessLoadedPackages) const;

	void ResetState();

private:
	FThreadSafeBool bEnabled = true;

	FThreadSafeCounter PendingDescCount;

	TSet<const FAsyncPackage*> PendingPackages;

	TArray<FAsyncPackage*> ReadyForTick;
	
	FThreadSafeBool bPriorityPackageLoaded = false;
};
//--[SilverPalace] End add by jianpeng.fan
...
class FAsyncLoadingThread final : public FRunnable, public IAsyncPackageLoader
{
	...
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	/** [ASYNC/GAME THREAD] Encapsulated state for highest-priority load preemption feature */
	FPriorityPackageLoadState PriorityPackageState;
	//--[SilverPalace] End add by jianpeng.fan
