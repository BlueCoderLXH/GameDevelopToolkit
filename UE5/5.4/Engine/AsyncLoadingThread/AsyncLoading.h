...
namespace EAsyncPackageYieldPoint
{
	enum Type
	{
		LoadImports,
...
template<int TimesThreshold>
struct FPackageYieldChecker
{
	bool CheckEvent(EAsyncPackageYieldPoint::Type YieldPoint)
	{
		if (++YieldTimes[YieldPoint] <= TimesThreshold)
		{
			return true;
		}
		else
		{
			YieldTimes[YieldPoint] = 0;
			return false;
		}
	}
...
protected:
	...
	int32 YieldTimes[EAsyncPackageYieldPoint::Count] = {};
};
...
public:
	...
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	FPackageYieldChecker<2> YieldChecker;
	//--[SilverPalace] End add by jianpeng.fan
