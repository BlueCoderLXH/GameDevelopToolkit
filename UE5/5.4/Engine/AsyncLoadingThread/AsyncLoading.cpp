...
{
	{
		...
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		if (PriorityPackageState.IsEnabled() && PriorityPackageState.IsPriorityPackage(Package))
		{
			PriorityPackageState.OnDescQueued();
		}
		//--[SilverPalace] End add by jianpeng.fan
...
void FAsyncLoadingThread::UpdateExistingPackagePriorities(FAsyncPackage* InPackage, TAsyncLoadPriority InNewPriority)
{
	...
	if (GEventDrivenLoaderEnabled)
	{
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		// 只在包的优先级需要提升时才重新入队
		if (PriorityPackageState.IsEnabled() && (PriorityPackageState.IsPriorityPackage(InPackage) || InNewPriority == FAsyncLoadEvent::RequestPriority_MAX))
		{
			EventQueue.RequeueEventsForPackage(InPackage->SerialNumber, InNewPriority);
		}
	...
	{
		{
			...
			//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
			// 先处理优先请求
			if (PriorityPackageState.IsEnabled() && PriorityPackageState.HasPendingDescs())
			{
				int32 WriteIdx = 0;
				for (int32 ReadIdx = 0; ReadIdx < QueuedPackages.Num(); ++ReadIdx)
				{
					if (QueueCopy.Num() < TimeSliceGranularity &&
						QueuedPackages[ReadIdx]->Priority == FAsyncLoadEvent::RequestPriority_MAX)
					{
						QueueCopy.Add(QueuedPackages[ReadIdx]);
						PriorityPackageState.OnDescConsumed();
						NumCopied++;
					}
					else
					{
						if (WriteIdx != ReadIdx)
						{
							QueuedPackages[WriteIdx] = QueuedPackages[ReadIdx];
						}
						++WriteIdx;
					}
				}
				QueuedPackages.SetNum(WriteIdx, EAllowShrinking::No);

				if (NumCopied == 0)
				{
					PriorityPackageState.ResetDescCount();
				}
			}
			if (NumCopied == 0)
			{
...
void FAsyncLoadingThread::QueueEvent_CreateLinker(FAsyncPackage* Package, int32 EventSystemPriority)
{
	...
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	EventSystemPriority = PriorityPackageState.AdjustEventPriority(Package, EventSystemPriority);
	//--[SilverPalace] End add by jianpeng.fan
...
void FAsyncPackage::Event_CreateLinker()
{
	...
	// ++[SilverPalace] Begin add by Minghao.He 2025/06/25
	TRACE_LOADTIME_CREATE_LINKER_SCOPE(this, GetPackagePath().GetSPDebugName())
	// --[SilverPalace] End add by Minghao.He 2025/06/25
	check(!Linker);
	...
	else
	{
		...
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		AsyncLoadingThread.PriorityPackageState.AddReadyPackage(this);
		//--[SilverPalace] End add by jianpeng.fan
	}
}
...
void FAsyncLoadingThread::QueueEvent_FinishLinker(FWeakAsyncPackagePtr WeakPtr, int32 EventSystemPriority)
{
	...
	if (Pkg)
	{
		...
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		EventSystemPriority = PriorityPackageState.AdjustEventPriority(Pkg, EventSystemPriority);
		//--[SilverPalace] End add by jianpeng.fan
		EventQueue.AddAsyncEvent(UserPriority, PackageSerialNumber, EventSystemPriority,
			TFunction<void(FAsyncLoadEventArgs& Args)>(
				[WeakPtr, this](FAsyncLoadEventArgs& Args)
...
void FAsyncPackage::Event_FinishLinker()
{
	...
	// ++[SilverPalace] Begin add by Minghao.He 2025/06/25
	TRACE_LOADTIME_FINISH_LINKER_SCOPE(this, GetPackagePath().GetSPDebugName())
	// --[SilverPalace] End add by Minghao.He 2025/06/25
	...
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	if ((Result == EAsyncPackageState::TimeOut || AsyncLoadingThread.PriorityPackageState.ShouldYield(this, EAsyncPackageYieldPoint::EventFinishLinker)) && !bLoadHasFailed)
	//--[SilverPalace] End add by jianpeng.fan
	{
	...
	if (!bLoadHasFailed)
	{
...

		{
			...
			for (int32 LocalImportIndex = ImportAddNodeIndex; LocalImportIndex < Linker->ImportMap.Num(); LocalImportIndex++)
			{
				...
				//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
				if ((LocalImportIndex % 50 == 0 && IsTimeLimitExceeded()) || AsyncLoadingThread.PriorityPackageState.ShouldYield(this, EAsyncPackageYieldPoint::EventFinishLinker))
				//--[SilverPalace] End add by jianpeng.fan
				{
					AsyncLoadingThread.QueueEvent_FinishLinker(FWeakAsyncPackagePtr(this), FAsyncLoadEvent::EventSystemPriority_MAX);
					return;
			...
			for (int32 LocalExportIndex = ExportAddNodeIndex; LocalExportIndex < Linker->ExportMap.Num(); LocalExportIndex++)
			{
				...
				//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
				if ((LocalExportIndex % 30 == 0 && IsTimeLimitExceeded()) || AsyncLoadingThread.PriorityPackageState.ShouldYield(this, EAsyncPackageYieldPoint::EventFinishLinker))
				//--[SilverPalace] End add by jianpeng.fan
				{
					AsyncLoadingThread.QueueEvent_FinishLinker(FWeakAsyncPackagePtr(this), FAsyncLoadEvent::EventSystemPriority_MAX);
					return;
...
void FAsyncPackage::Event_StartImportPackages()
{
...

	{
		FScopedAsyncPackageEvent Scope(this);
		// ++[SilverPalace] Begin add by Minghao.He 2025/06/25
		TRACE_LOADTIME_LOAD_IMPORT_SCOPE(this, GetPackagePath().GetSPDebugName())
		// --[SilverPalace] End add by Minghao.He 2025/06/25
...
EAsyncPackageState::Type FAsyncPackage::LoadImports_Event()
{
	...
	{
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		if (AsyncLoadingThread.PriorityPackageState.ShouldYield(this, EAsyncPackageYieldPoint::LoadImports)) break;
		//--[SilverPalace] End add by jianpeng.fan
		...
		{
			//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
			if (AsyncLoadingThread.PriorityPackageState.IsEnabled() && AsyncLoadingThread.PriorityPackageState.IsPriorityPackage(Desc))
			{
				AsyncLoadingThread.UpdateExistingPackagePriorities(PendingPackage, Desc.Priority);
			}
			//--[SilverPalace] End add by jianpeng.fan
...
{
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	check(Package->AsyncPackageLoadingState == EAsyncPackageLoadingState::WaitingForImportPackages || Package->AsyncPackageLoadingState == EAsyncPackageLoadingState::SetupImports);
	//--[SilverPalace] End add by jianpeng.fan
...
{
	{
		FScopedAsyncPackageEvent Scope(this);
		// ++[SilverPalace] Begin add by Minghao.He 2025/06/25
		TRACE_LOADTIME_SETUP_IMPORT_SCOPE(this, GetPackagePath().GetSPDebugName())
		// --[SilverPalace] End add by Minghao.He 2025/06/25
		//@todo we need to time slice this, it runs to completion at the moment
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		EAsyncPackageState::Type State = SetupImports_Event();
		if (State != EAsyncPackageState::Complete && AsyncLoadingThread.PriorityPackageState.IsEnabled())
		{
			AsyncLoadingThread.QueueEvent_SetupImports(this, FAsyncLoadEvent::EventSystemPriority_MAX);
			return;
		}
		verify(State == EAsyncPackageState::Complete);
		//--[SilverPalace] End add by jianpeng.fan
	}
...
EAsyncPackageState::Type FAsyncPackage::SetupImports_Event()
{
	...
	{
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		if (AsyncLoadingThread.PriorityPackageState.ShouldYield(this, EAsyncPackageYieldPoint::SetupImports)) break;
		//--[SilverPalace] End add by jianpeng.fan
...
{
	{
		FScopedAsyncPackageEvent Scope(this);
		// ++[SilverPalace] Begin add by Minghao.He 2025/06/25
		TRACE_LOADTIME_SETUP_EXPORT_SCOPE(this, GetPackagePath().GetSPDebugName())
		// --[SilverPalace] End add by Minghao.He 2025/06/25
...
EAsyncPackageState::Type FAsyncPackage::SetupExports_Event()
{
	...
	{
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		if (AsyncLoadingThread.PriorityPackageState.ShouldYield(this, EAsyncPackageYieldPoint::SetupExports)) break;
		//--[SilverPalace] End add by jianpeng.fan
...
void FAsyncPackage::EventDrivenCreateExport(int32 LocalExportIndex)
{
	...
	// ++[SilverPalace] Begin edit by Minghao.He 2025/06/20
	TRACE_LOADTIME_CREATE_EXPORT_SCOPE(this, &Export.Object, Linker->GetPackagePath().GetSPDebugName());
	// --[SilverPalace] End edit by Minghao.He 2025/06/20
...
void FAsyncPackage::EventDrivenSerializeExport(int32 LocalExportIndex)
{
	...
	if (Object && Object->HasAnyFlags(RF_NeedLoad))
	{
		...
		// ++[SilverPalace] Begin edit by Minghao.He 2025/06/20
		TRACE_LOADTIME_SERIALIZE_EXPORT_SCOPE(Object, Export.SerialSize, GetPackagePath().GetSPDebugName());
		// --[SilverPalace] End edit by Minghao.He 2025/06/20
...
EAsyncPackageState::Type FAsyncPackage::ProcessImportsAndExports_Event()
{
	...
	while (!IsTimeLimitExceeded() && bDidSomething)
	{
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		// 如果有更优先的请求暂缓当前加载
		if (AsyncLoadingThread.PriorityPackageState.ShouldYield(this, EAsyncPackageYieldPoint::ProcessImportsAndExports))
		{
...
//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
bool FPriorityPackageLoadState::InitializeFromCommandLine()
{
	if (FParse::Param(FCommandLine::Get(), TEXT("DisablePriorityLoad")))
	{
		bEnabled = false;
		UE_LOG(LogStreaming, Log, TEXT("[PriorityLoad] CommandLine -DisablePriorityLoad => FORCE OFF"));
	}

	return bEnabled;
}

void FPriorityPackageLoadState::OnPackageAdded(const FAsyncPackage* Package)
{
	if (IsEnabled() && Package && IsPriorityPackage(Package))
	{
		PendingPackages.Add(Package);
	}
}

void FPriorityPackageLoadState::OnPackageReleased(const FAsyncPackage* Package)
{
...
bool FPriorityPackageLoadState::IsPriorityPackage(const FAsyncPackage* Package)
{
	return Package && Package->GetPriority() == FAsyncLoadEvent::RequestPriority_MAX;
}

bool FPriorityPackageLoadState::IsPriorityPackage(const FAsyncPackageDesc& Package)
{
	return Package.Priority == FAsyncLoadEvent::RequestPriority_MAX;
}

int32 FPriorityPackageLoadState::AdjustEventPriority(const FAsyncPackage* Package, int32 Original) const
{
	if (!IsEnabled() || !Package)
	{
		return Original;
	...
	// 将普通请求约束到 NormalIsolation 以下，确保不阻塞高优事件
	if (!IsPriorityPackage(Package))
	{
		return Original - FAsyncLoadEvent::NormalIsolation;
	}
	
	// 将高优请求的节点优先级映射到 [MAX - PriorityIsolation, MAX]
	return FMath::Max( FAsyncLoadEvent::EventSystemPriority_MAX - FAsyncLoadEvent::PriorityIsolation + (Original < 0 ? Original : 0), Original);
}

bool FPriorityPackageLoadState::ShouldYield(FAsyncPackage* Package, EAsyncPackageYieldPoint::Type YieldPoint) const
{
	if (!IsEnabled() || !Package || IsPriorityPackage(Package))
	{
		return false;
	...
	return (HasPendingDescs() || PendingPackages.Num() > 0) && Package->YieldChecker.CheckEvent(YieldPoint);
}

void FPriorityPackageLoadState::AddReadyPackage(FAsyncPackage* Package)
{
	if (IsEnabled() && IsPriorityPackage(Package) && !ReadyForTick.Contains(Package))
	{
		ReadyForTick.Add(Package);
	}
}

FAsyncPackage* FPriorityPackageLoadState::PopNextReadyPackage(FAsyncPackage* NormalQueueHead)
{
	if (ReadyForTick.Num() > 0)
	{
		return ReadyForTick[0];
	}
	return NormalQueueHead;
}

void FPriorityPackageLoadState::RemoveReadyPackage(FAsyncPackage* Package)
{
	ReadyForTick.RemoveSingle(Package);
}

void FPriorityPackageLoadState::SortLoadedPackages(TArray<FAsyncPackage*>& InProcessLoadedPackages) const
{
	if (!IsEnabled()) return;
	
	TArray<FAsyncPackage*> HighPriority, NormalPriority;
	HighPriority.Reserve(InProcessLoadedPackages.Num());
	NormalPriority.Reserve(InProcessLoadedPackages.Num());

	for (FAsyncPackage* Package : InProcessLoadedPackages)
	{
		(Package->GetPriority() == MAX_int32 ? HighPriority : NormalPriority).Add(Package);
	}

	// 在保持先后顺序的情况下将优先请求排在前面
	InProcessLoadedPackages = MoveTemp(HighPriority);
	InProcessLoadedPackages.Append(MoveTemp(NormalPriority));
}

void FPriorityPackageLoadState::ResetState()
{
	PendingDescCount.Reset();
	PendingPackages.Reset();
	ReadyForTick.Reset();
	bPriorityPackageLoaded = false;
}
//--[SilverPalace] End add by jianpeng.fan
...
void FAsyncLoadingThread::InsertPackage(FAsyncPackage* Package, bool bReinsert, EAsyncPackageInsertMode InsertMode)
{
...

	{
		...
		if (GEventDrivenLoaderEnabled)
		{
			...
			//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
			PriorityPackageState.OnPackageAdded(Package);
			//--[SilverPalace] End add by jianpeng.fan
		}
		else
		{
...
void FAsyncLoadingThread::AddToLoadedPackages(FAsyncPackage* Package)
{
	...
	if (!LoadedPackages.Contains(Package))
	{
		...
		//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
		if (FPriorityPackageLoadState::IsPriorityPackage(Package))
		{
			PriorityPackageState.SignalPackageLoaded();
		}
		//--[SilverPalace] End add by jianpeng.fan
	}
}
...
EAsyncPackageState::Type FAsyncLoadingThread::ProcessAsyncLoading(int32& OutPackagesProcessed, bool bUseTimeLimit /*= false*/, bool bUseFullTimeLimit /*= false*/, float TimeLimit /*= 0.0f*/, FFlushRequest& FlushRequest)
{
	...
	if (GEventDrivenLoaderEnabled)
	{
		...
		while (true)
		{
			...
			//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
			if (bDidSomething && !PriorityPackageState.HasReadyPackages())
			{
				continue;
			}
			//--[SilverPalace] End add by jianpeng.fan
			if (AsyncPackagesReadyForTick.Num())
			{
				...
				//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
				// 如果有优先请求先处理
				FAsyncPackage* Package = PriorityPackageState.PopNextReadyPackage(AsyncPackagesReadyForTick.Num() > 0 ? AsyncPackagesReadyForTick[0] : nullptr);
				//--[SilverPalace] End add by jianpeng.fan
				...
				{
					{
						...
						//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
						AsyncPackagesReadyForTick.RemoveSingle(Package);
						PriorityPackageState.OnPackageReleased(Package);
						PriorityPackageState.RemoveReadyPackage(Package);
						//--[SilverPalace] End add by jianpeng.fan
					}
			...
			{
				//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
				if (PriorityPackageState.HasPendingDescs())
				{
					return EAsyncPackageState::TimeOut;
				}
				//--[SilverPalace] End add by jianpeng.fan
...
EAsyncPackageState::Type FAsyncLoadingThread::ProcessLoadedPackages(bool bUseTimeLimit, bool bUseFullTimeLimit, double TimeLimit, bool& bDidSomething, const FFlushRequest& FlushRequest)
{
	...
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	if(PriorityPackageState.IsEnabled())
	{
		PriorityPackageState.SortLoadedPackages(LoadedPackagesToProcess);
		PriorityPackageState.ClearPackageLoadedSignal();
	}
	//--[SilverPalace] End add by jianpeng.fan
	...
	for (int32 PackageIndex = 0; PackageIndex < LoadedPackagesToProcess.Num() && !IsAsyncLoadingSuspendedInternal(); ++PackageIndex)
	{
			...
			|| GEventDrivenLoaderEnabled)
		{
			...
			//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
			if (PriorityPackageState.IsEnabled() && !FPriorityPackageLoadState::IsPriorityPackage(Package) && PriorityPackageState.HasLoadedPackage())
			{
...
//++[SilverPalace] Begin add by wangjingyi 2025/03/06
				if (!LoadedAsset->HasAnyFlags(RF_LoadCompleted))
				{
					UE_LOG(LogTemp, Warning, TEXT("%s has not been fully loaded but called OnAssetLoaded"), *LoadedAsset->GetName());
				}
//--[SilverPalace] End add by wangjingyi
	...
	, Thread(nullptr)
{
	...
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	PriorityPackageState.InitializeFromCommandLine();
	//--[SilverPalace] End add by jianpeng.fan
...
void FAsyncLoadingThread::CancelAsyncLoadingInternal()
{
	...
	//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
	PriorityPackageState.ResetState();
	//--[SilverPalace] End add by jianpeng.fan
...
EAsyncPackageState::Type FAsyncPackage::CreateLinker()
{
	...
	if (Linker == nullptr)
	{
		...
		if (!Linker)
		{
			...
			//++[SilverPalace] Begin add by dekun.li 2026/04/18
			TRACE_CPUPROFILER_EVENT_SCOPE(AsyncPackage_CreateLinkerAsync);
			//--[SilverPalace] End add by dekun.li
			FUObjectSerializeContext* LoadContext = GetSerializeContext();
			if (GEventDrivenLoaderEnabled)
			{
				FWeakAsyncPackagePtr WeakPtr(this);
...
//++[SilverPalace] Begin add by wangjingyi 2024/09/05
		// Original UE code here is to check !ExistingPackage->bHasBeenFullyLoaded,
		// but UPackage::bHasBeenFullyLoaded is not meaning the package is fully loaded!
		// The objects under the package may just be called CreateExport and not Preloaded!
		// This fix ensures all the Imports is Preloaded (by force adding dependency)
		// but the loading of this package itself maybe delayed
		if (ExistingPackage && !PendingImportedPackages.Contains(ImportPackageFName) && !IsNativeCodePackage(ExistingPackage))
//--[SilverPalace] End add by wangjingyi
		{
...
EAsyncPackageState::Type FAsyncPackage::PostLoadDeferredObjects(double InTickStartTime, bool bInUseTimeLimit, double InOutTimeLimit)
{
...

	{
		...
		{
			//++[SilverPalace] Begin add by jianpeng.fan 2026/06/16
			if ((DeferredPostLoadIndex % 10) == 0 &&
				AsyncLoadingThread.PriorityPackageState.IsEnabled() && 
				!FPriorityPackageLoadState::IsPriorityPackage(this) && 
				AsyncLoadingThread.PriorityPackageState.HasLoadedPackage()
			)
			{
				break;
			}
